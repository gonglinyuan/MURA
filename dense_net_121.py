...
def dense_net_121(img_rows, img_cols, color_type=1, nb_dense_block=4, growth_rate=32, nb_filter=64, reduction=0.5,
                  dropout_rate=0.0, weight_decay=0.0, dropout_fc=0.0, weights_path=None, freeze_dense_block_num=0):
    '''
    ...
    for block_idx in range(nb_dense_block - 1):
        stage = block_idx + 2
        if block_idx < freeze_dense_block_num:
            x, nb_filter = dense_block(x, stage, nb_layers[block_idx], nb_filter, growth_rate, dropout_rate=dropout_rate,
                                   weight_decay=weight_decay, trainable=False)
        else:
            x, nb_filter = dense_block(x, stage, nb_layers[block_idx], nb_filter, growth_rate, dropout_rate=dropout_rate,
                                   weight_decay=weight_decay, trainable=True)
...
def conv_block(x, stage, branch, nb_filter, dropout_rate=None, weight_decay=1e-4, trainable=True):
    ...
    x = BatchNormalization(epsilon=eps, axis=concat_axis, name=conv_name_base + '_x1_bn',
                           beta_regularizer=l2(weight_decay), gamma_regularizer=l2(weight_decay), trainable=trainable)(x)
    x = Scale(axis=concat_axis, name=conv_name_base + '_x1_scale', trainable=trainable)(x)
    ...
    x = BatchNormalization(epsilon=eps, axis=concat_axis, name=conv_name_base + '_x2_bn',
                           beta_regularizer=l2(weight_decay), gamma_regularizer=l2(weight_decay), trainable=trainable)(x)
    x = Scale(axis=concat_axis, name=conv_name_base + '_x2_scale', trainable=trainable)(x)
    x = Activation('relu', name=relu_name_base + '_x2', trainable=trainable)(x)
    x = ZeroPadding2D((1, 1), name=conv_name_base + '_x2_zeropadding', trainable=trainable)(x)
    x = Convolution2D(nb_filter, 3, 3, name=conv_name_base + '_x2', bias=False, kernel_regularizer=l2(weight_decay), trainable=trainable)(x)
...
def dense_block(x, stage, nb_layers, nb_filter, growth_rate, dropout_rate=None, weight_decay=1e-4,
                grow_nb_filters=True, trainable=True):
    ...
    for i in range(nb_layers):
        branch = i + 1
        x = conv_block(concat_feat, stage, branch, growth_rate, dropout_rate, weight_decay, trainable=trainable)
