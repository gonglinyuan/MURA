...
def conv_block(x, stage, branch, nb_filter, dropout_rate=None, weight_decay=1e-4, trainable=True):
    ...
    x = Convolution2D(nb_filter, 3, 3, name=conv_name_base + '_x2', bias=False, kernel_regularizer=l2(weight_decay),
...
def transition_block(x, stage, nb_filter, compression=1.0, dropout_rate=None, weight_decay=1E-4, trainable=True):
    ...
    x = BatchNormalization(epsilon=eps, axis=concat_axis, name=conv_name_base + '_bn',
                           beta_regularizer=l2(weight_decay), gamma_regularizer=l2(weight_decay), trainable=trainable)(
        x)
    x = Scale(axis=concat_axis, name=conv_name_base + '_scale', trainable=trainable)(x)
