...
def run_train():
    ...
    path_log = '../../trained_models/' + timestamp + '-dualpathnet131-adam-augslight/tb'
    model_name = 'DUALPATHNET131'
    model_pretrained = True
    batch_size = 16
    epoch_num = 100
    path_model = '../../trained_models/' + timestamp + '-dualpathnet131-adam-augslight/m-' + timestamp + '.pth.tar'
    ...
    train.train(
        ...
        checkpoint=None,
        device=device,
            transform_train=data_augmentation.augment_transform_slight(target_mean=np.array([124 / 255, 117 / 255, 104 / 255]), 
                                                                       target_std=np.array([1 / (.0167 * 255)] * 3)),
        transform_valid=data_augmentation.valid_transform(target_mean=np.array([124 / 255, 117 / 255, 104 / 255]), 
                                                          target_std=np.array([1 / (.0167 * 255)] * 3)),
    ...
    train.test(
        ...
        batch_size=batch_size,
        device=device,
        transform=data_augmentation.valid_transform(target_mean=np.array([124 / 255, 117 / 255, 104 / 255]), 
                                                    target_std=np.array([1 / (.0167 * 255)] * 3))
    )
