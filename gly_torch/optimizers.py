...
def adam_optimizers_tiny(parameters):
    optimizer = optim.Adam(parameters, lr=0.000005, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-5)
    scheduler = ReduceLROnPlateau(optimizer, factor=0.1, patience=5, mode='min', verbose=True)
    return optimizer, scheduler


def adam_optimizers_tiny6(parameters):
    optimizer = optim.Adam(parameters, lr=1e-06, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-5)
    scheduler = ReduceLROnPlateau(optimizer, factor=0.1, patience=5, mode='min', verbose=True)
    return optimizer, scheduler


def adam_optimizers_tiny7(parameters):
    optimizer = optim.Adam(parameters, lr=1e-07, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-5)
    scheduler = ReduceLROnPlateau(optimizer, factor=0.1, patience=5, mode='min', verbose=True)
    return optimizer, scheduler
